    ...
    <div className="relative">
      ...
      <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between pointer-events-none z-10 px-2">
        <button
          ...
          className="pointer-events-auto w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-all disabled:opacity-30"
          style={{
      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex gap-4 touch-pan-y">
          {products.map((product, index) => (
            <div
              key={product.id}
              className="flex-none w-[85%] sm:w-[45%] md:w-[calc(33.333%-0.67rem)] lg:w-[calc(33.333%-0.67rem)]"
            >
              <ProductRevealAnimation delay={index * 0.05}>
                <div
                  className="rounded-[var(--radius-lg)] border overflow-hidden h-full flex flex-col"
                  style={{
                    backgroundColor: 'var(--color-card)',
      ...
      <div className="flex md:hidden justify-center gap-2 mt-4">
        {products.map((_, index) => (
          <button
            ...
            style={{
              backgroundColor: emblaApi?.selectedScrollSnap() === index
                ? 'var(--color-primary)'
                : 'var(--color-muted)',
            }}
          />
